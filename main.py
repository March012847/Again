...
class DownloaderUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.url_input = TextInput(hint_text="Paste Spotify or YouTube URL", multiline=False, size_hint_y=None, height=50)
        self.add_widget(self.url_input)

        self.log_label = Label(text="Ready.", size_hint_y=None, height=250)
        self.add_widget(self.log_label)

        dl_btn = Button(text="Download", size_hint_y=None, height=55)
    ...
    def start_download(self, *args):
        ...
        if not url:
            ...
            return
        threading.Thread(target=self.download_handler, args=(url,)).start()
    ...
    def download_spotify(self, url):
        ...
        cmd = [OSPY, "-m", "spotdl", "download", "--ffmpeg", FFMPEG, "--output", os.path.join(DOWNLOAD_DIR, "{title}.{ext}"), url]
        try:
